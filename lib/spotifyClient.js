...
class SpotifyClient {
    /**
     * @param {{clientId:string, clientSecret:string, refreshToken:string, redirectUri?:string, log?: any}} opts
     */
    constructor(opts) {
     ...
     * @param {string} method
     * @param {string} path - e.g. "/me/player"
     * @param {{query?:object, body?:any, allowNoContent?:boolean}} [opts]
     */
    async api(method, path, opts = {}) {
        ...
        if (res.status === 429) {
            ...
            if (this.log?.warn) this.log.warn(`Spotify rate limited (429). Retrying after ${retryAfter}s`);
            await delay(Math.max(1, retryAfter) * 1000);
            return this.api(method, path, opts);
        }

        // 204 No Content is valid for several player endpoints and also when no active playback
        if (res.status === 204) {
            return null;
        }
    ...
    async playUri(uri, { deviceId } = {}) {
        ...
        // If it's a track URI, use `uris`, otherwise use `context_uri`
